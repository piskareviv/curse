    ...
    for (size_t batch_sz = 1; batch_sz <= 1000; batch_sz *= 2) {
        {
            ...
            std::unique_ptr<BatchStream> stream = std::move(reader) >= count;
            ...
            const std::string expected_result = "10\n";
        ...
        }
        {
            ...
            std::unique_ptr<BatchStream> stream = std::move(reader) >= select >= count;
